...
class STTNInpaint:
    ...
    def __call__(self, input_frames: List[np.ndarray], input_mask: np.ndarray):
        ...
        split_h = max(80, int(frame_info['H_ori'] * 0.15))
        ...
        if inpaint_area:
            for j in range(len(frames_hr)):
                ...
                for k in range(len(inpaint_area)):
                    ...
                    mask_area = m[inpaint_area[k][0]:inpaint_area[k][1], :]

                    # === 新增：soft mask 羽化 ===
                    mask_uint8 = (mask_area * 255).astype(np.uint8)
                    kernel = np.ones((5, 5), np.uint8)
                    mask_soft = cv2.dilate(mask_uint8, kernel, iterations=1)
                    mask_soft = cv2.GaussianBlur(mask_soft, (0, 0), sigmaX=3, sigmaY=3)
                    ...
                    frame[inpaint_area[k][0]:inpaint_area[k][1], :, :] = \
                        mask_soft * comp + (1 - mask_soft) * frame[inpaint_area[k][0]:inpaint_area[k][1], :, :]
                # 将最终帧添加到列表
...
class STTNVideoInpaint:
    ...
    def __call__(self, input_mask=None, input_sub_remover=None, tbar=None, mask_getter=None):
        reader = None
        writer = None
        try:
            # 读取视频帧信息
            ...
            split_h = max(120, int(frame_info['H_ori'] * 0.3))
                    ...
                    for j in range(valid_frames_count):
                        ...
                        for k in range(len(inpaint_area)):
                            if j < len(comps[k]):  # 确保索引有效
                                ...
                                global_frame_no = start_f + j + 1  # 你这套代码帧号从1开始更符合 main.py 的逻辑

                                if mask_getter is not None:
                                    m = mask_getter(global_frame_no)         # 返回 HxW uint8(0/255)
